...
async def analyze_image(file: UploadFile = File(...), session_id: str = "auto"):
    ...
    try:
        ...
        # 피부색 수치 (MLP) 예측
        cheek_rgb = np.array(cheek_rgb, dtype=np.float32) / 255.0
        cheek_lab = np.array(cheek_lab, dtype=np.float32)
        forehead_rgb = np.array(forehead_rgb, dtype=np.float32) / 255.0
        forehead_lab = np.array(forehead_lab, dtype=np.float32)
        ...
        save_result(session_id, predicted_season, combined_confidence.item(), 
                   cheek_rgb, cheek_lab, forehead_rgb, forehead_lab)
