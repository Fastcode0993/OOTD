...
from tabs.tab_personal_color import PersonalColorTab
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self._d=DatasetTab(); self._p=PreprocessTab(); self._t=TrainTab(); self._i=InferenceTab(); self._a=AutoLabelTab(); self._pc=PersonalColorTab()
        ...
        self._tabs.addTab(self._a,"🏷️  5. 자동 라벨링"); self._tabs.addTab(self._pc,"🎨  6. 퍼스널 컬러 분석"); ml.addWidget(self._tabs)
    ...
    def _wire(self):
        ...
        self._d.dataset_loaded.connect(
            lambda p: self.statusBar().showMessage(f"데이터셋 로드됨: {p}"))
        # ★ 전처리 완료 → 학습 탭 자동 연결 (경로, 모드명, output_format)
        self._p.preprocess_done.connect(self._t.set_preprocessed_dir)
        self._p.preprocess_done.connect(
            lambda p, m, f: self.statusBar().showMessage(
                f"전처리 완료 [{m}][{f.upper()}] → 학습 탭 연결됨: {p}"))
    def _style(self):
        self.setStyleSheet("""
    ...
    def closeEvent(self, event):
        for w in [getattr(self._d,"_worker",None), getattr(self._p,"_worker",None),
                  getattr(self._t,"_worker",None), getattr(self._i,"_worker",None),
                  getattr(self._a,"_worker",None), getattr(self._pc,"_worker",None)]:
            if w and hasattr(w,"stop") and w.isRunning():
                w.stop()
                w.wait(5000)
        event.accept()