...
def _gray_world_wb(bgr: np.ndarray) -> np.ndarray:
    """
    Gray World 화이트 밸런스 보정.
    채널별 평균을 전체 평균으로 스케일 보정 (보정 범위 0.80~1.25 제한).
    조명 색온도 편향(황색/청색 캐스트) 제거에 효과적.
    """
    bgr_f = bgr.astype(np.float32)
    means = bgr_f.mean(axis=(0, 1))           # [B_mean, G_mean, R_mean]
    gray  = float(means.mean())
    scale = np.clip(gray / (means + 1e-6), 0.80, 1.25).astype(np.float32)
    return np.clip(bgr_f * scale, 0, 255).astype(np.uint8)


def _clahe_l_channel(bgr: np.ndarray) -> np.ndarray:
    """
    LAB L채널에 CLAHE 적용 → 조도 불균일 완화.
    clipLimit=2.0 으로 과보정 방지, 색상(a/b) 채널은 건드리지 않음.
    """
...
def _extract_features_from_skin(skin_px_bgr: np.ndarray,
    ...
    raw = [
        ...
        float(abs(b_delta) >= 0.3),           # corrected flag (임계값 동기화)
...
class FacePreprocessor:
    ...
    def _process_mediapipe(self, bgr: np.ndarray) -> FaceDetectionResult:
        ...
        # ── b* 보정 (논문 기준값으로 정규화, 임계값 0.3으로 강화) ──────
        work_bgr = bgr
        if abs(b_delta) >= 0.3:
            work_bgr = _b_correct_cpu(bgr, b_delta)

        # ── CLAHE: 조도 불균일 완화 ───────────────────────────────────
        work_bgr = _clahe_l_channel(work_bgr)
    ...
    def _process_haarcascade(self, bgr: np.ndarray) -> FaceDetectionResult:
        ...
        # b* 보정 + CLAHE (MediaPipe 경로와 동일하게 적용)
        if abs(b_delta) >= 0.3:
            crop = _b_correct_cpu(crop, b_delta)
        crop = _clahe_l_channel(crop)

        # 보정 후 피부 픽셀 재추출
        sm2      = self._ycrcb_skin_mask(crop)
        skin_px  = crop[sm2 == 255] if sm2.sum() > 0 else skin_px
